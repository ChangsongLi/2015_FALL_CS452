...
public class QLearning {
	...
	QLearning(String file) {
		File f = new File("QLearning.txt");
		File fReward = new File("QLearningReward.txt");
	    try {
			f.createNewFile();
	...
	public void QValue() {
		...
		for (int episodes = 1; episodes <= 2000; episodes++) {
			...
			while (!sTerminal) {
				int direction = chooseEpsilonGreedyAction(currentPoint.x, currentPoint.y, episodes);
				Point newPosition;
				// Right now not on Ice
				if(!isIce(currentPoint.x, currentPoint.y)){
					newPosition = getNewPosition(currentPoint, direction);
				}else{
					newPosition = getNewPositionOnIce(currentPoint, direction);
				}
				// is still in grid
				if (canGoThere(newPosition.x, newPosition.y)) {
					...
					if (isHole(newPosition.x, newPosition.y)) {
						reward = reward - 100;
						double currentQValue = getCurrentQValue(currentPoint.x, currentPoint.y, direction);
						double nextStateQValue = grid[currentPoint.x][currentPoint.y].getMax();
						double newValue = currentQValue
								+ ALPHA * (HOLECOST + (GAMMA * nextStateQValue) - currentQValue);
						updateValue(currentPoint.x, currentPoint.y, direction, newValue);
					} else {
						int cost = -1;
						...
						double currentQValue = getCurrentQValue(currentPoint.x, currentPoint.y, direction);
						double nextStateQValue = grid[newPosition.x][newPosition.y].getMax();
